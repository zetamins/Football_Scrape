...
@Composable
fun PerformanceTab(insights: InsightsPerformance, homeTeam: String, awayTeam: String) {
    Column(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
        AttackProfileRadar(insights, homeTeam, awayTeam)
...
private fun AttackProfileRadar(insights: InsightsPerformance, homeTeam: String, awayTeam: String) {
    ...
    SectionCard("Attack profile") {
        val axes = listOf(
            RadarAxis("xG", (insights.homeXgEstimate?.xgFor ?: 0.0).toFloat(), (insights.awayXgEstimate?.xgFor ?: 0.0).toFloat(), maxValue = 3f),
...
@Composable
private fun CoreStatsSection(insights: InsightsPerformance, homeTeam: String, awayTeam: String) {
    val hasCore = insights.homeXgEstimate != null || insights.homeShotsEstimate != null ||
    ...
    SectionCard("Season estimates") {
        insights.homeXgEstimate?.let { h ->
            insights.awayXgEstimate?.let { a ->
                BarComparison("xG for", h.xgFor.toFloat(), a.xgFor.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam ${h.xgFor}", "$awayTeam ${a.xgFor}")
                Spacer(Modifier.height(8.dp))
                BarComparison("xG against", h.xgAgainst.toFloat(), a.xgAgainst.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam ${h.xgAgainst}", "$awayTeam ${a.xgAgainst}")
                Spacer(Modifier.height(8.dp))
            }
        }
        insights.homeShotsEstimate?.let { h ->
            insights.awayShotsEstimate?.let { a ->
                BarComparison("Shots (last 10)", h.shotsFor.toFloat(), a.shotsFor.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam ${h.shotsFor}", "$awayTeam ${a.shotsFor}")
                Spacer(Modifier.height(8.dp))
                BarComparison("Shots on target (last 10)", h.shotsOnTargetFor.toFloat(), a.shotsOnTargetFor.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam ${h.shotsOnTargetFor}", "$awayTeam ${a.shotsOnTargetFor}")
                Spacer(Modifier.height(8.dp))
            }
        }
        insights.homeBigChancesEstimate?.let { h ->
            insights.awayBigChancesEstimate?.let { a ->
                BarComparison("Big chances created", h.bigChancesCreatedFor.toFloat(), a.bigChancesCreatedFor.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam ${h.bigChancesCreatedFor}", "$awayTeam ${a.bigChancesCreatedFor}")
                Spacer(Modifier.height(8.dp))
                BarComparison("Big chances missed", h.bigChancesMissedFor.toFloat(), a.bigChancesMissedFor.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam ${h.bigChancesMissedFor}", "$awayTeam ${a.bigChancesMissedFor}")
                Spacer(Modifier.height(8.dp))
            }
        }
        insights.homeCornersEstimate?.let { h ->
            insights.awayCornersEstimate?.let { a ->
                BarComparison("Corners (last 10)", h.cornersFor.toFloat(), a.cornersFor.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam ${h.cornersFor}", "$awayTeam ${a.cornersFor}")
            }
        }
    }
}
...
@Composable
private fun AdvancedStatsSection(insights: InsightsPerformance, homeTeam: String, awayTeam: String) {
...
@Composable
private fun AdvancedBar(label: String, home: Int, away: Int, homeTeam: String, awayTeam: String) {
    BarComparison(label, home.toFloat(), away.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam $home", "$awayTeam $away")
    Spacer(Modifier.height(8.dp))
}

@Composable
private fun AdvancedBarDouble(label: String, home: Double, away: Double, homeTeam: String, awayTeam: String) {
    BarComparison(label, home.toFloat(), away.toFloat(), AppTheme.colors.homeSeries, AppTheme.colors.awaySeries, "$homeTeam $home", "$awayTeam $away")
    Spacer(Modifier.height(8.dp))
}

@Composable
private fun PossessionMatchupSection(insights: InsightsPerformance, homeTeam: String, awayTeam: String) {
