...
fun SquadTab(
    ...
    squadStrength: SquadStrengthInfo?,
    label: String,
) {
...
private fun TopPerformersSection(profile: TeamProfileData) {
    ...
    SectionCard("Top performers") {
        ...
        profile.topDefenders?.takeIf { it.isNotEmpty() }?.let { list ->
            ...
            Spacer(Modifier.height(4.dp))
            RankedBarList(
                list.map { RankedEntry(it.name, it.tacklesMade, "${it.tacklesMade}T/${it.interceptions}I") },
    ...
    SectionCard("Form by role (ranked by minutes)") {
        profile.midfieldersForm?.takeIf { it.isNotEmpty() }?.let { list ->
            ...
            PlayerStatRows(
                list.map { m ->
                    m.name to
                        "${m.totalMinutes}min in ${m.matchesInSquad} (${m.starts} starts), ${m.goals}g/${m.assists}a"
                },
            )
            Spacer(Modifier.height(8.dp))
        }
